...
Viro_RNA <- Viro %>% filter(family %in% rna_families)

Viro_DNA <- Viro %>% filter(!family %in% rna_families)
```

```{r}
tbl <- table(Viro_RNA$family)
...
k <- 1
for (f in families){
  p <- pctidInScoreIntervals(Viro_RNA, f, title=f)
  ggsave(file=paste("img_families/RNAs/", f, "_", k, ".svg", sep=""), plot=p, width=10, height=10)
  k <- k +1
...
k <- 1
for (f in families){
  p <- pctidInScoreIntervals(Viro_DNA, f, title=f)
  ggsave(file=paste("img_families/DNAs/", f, "_", k, ".svg", sep=""), plot=p, width=10, height=10)
  k <- k +1
...
```{r}
#a toy example
#corona <- Viro_RNA %>% filter(family == "Coronaviridae")

#x <- corona %>% group_by(pctid,score_intervals, family) %>% summarize(n=n(), log10n = log10(n() + 1))
#x <- transform(x, family = factor(family, levels = names(tbl)))
#y <- x[with(x, order(pctid, -n)), ]

#sums <- rev(aggregate(n ~ pctid, y, cumsum))
#sums <- unlist(sums$n)
#y$cumsum <- sums
#y$log10_old <- log10(y$cumsum+1)

#cumm_difference
#test <- c(3.10, 3.13, 3.14)
...
#per families
for (f in families){
  x <- readDfSQL(con, "FamilySections", family = f, columns = c("Sra","Score", "PctId", "Family"), dataframe = T)
  colnames(x) <- tolower(colnames(x))
  
  p <- pctidInScoreIntervals(x, f, title=f)
  ggsave(file=paste("img_families/", f, ".svg", sep=""), plot=p, width=10, height=10)
  
}

#TEST
x <- readDfSQL(con, "FamilySections", family = "Coronaviridae", columns = c("Sra","Score", "PctId", "Family"),
               score=79,
               dataframe = T)
colnames(x) <- tolower(colnames(x))
corona <- pctidInScoreIntervals(x, "Coronaviridae", title="Coronaviridae")

x <- readDfSQL(con, "FamilySections", family = "Picornaviridae", columns = c("Sra","Score", "PctId", "Family"), dataframe = T)
colnames(x) <- tolower(colnames(x))
picorna <- pctidInScoreIntervals(x, "Picornaviridae", title="Picornaviridae")

x <- readDfSQL(con, "FamilySections", family = "Papillomaviridae", columns = c("Sra","Score", "PctId", "Family"), dataframe = T)
colnames(x) <- tolower(colnames(x))
papillo <- pctidInScoreIntervals(x, "Papillomaviridae", title="Papillomaviridae")

x <- readDfSQL(con, "FamilySections", family = "Alloherpesviridae", columns = c("Sra","Score", "PctId", "Family"), dataframe = T)
colnames(x) <- tolower(colnames(x))
allo <- pctidInScoreIntervals(x, "Alloherpesviridae", title="Alloherpesviridae")

ggarrange(corona, picorna, papillo, allo, 
          labels = c("A", "B", "C", "D"),
          ncol = 2, nrow = 2)
```
