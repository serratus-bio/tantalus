...
```{r}
vert_sra <- readSraRunInfo("../sra/mamm_SraRunInfo.csv")

mamm_serratus <- getLoadedSummaryFiles(mamm_sra@runInfo$Run, "../summary_files/")

mamm_sra@runInfo <- mamm_sra@runInfo %>% filter(Run %in% mamm_serratus@header$sra)

mamm_atys <- Atys(Serratus = mamm_serratus,
                  SRA = mamm_sra,
                  GENOME="")

print("Save to rds")
saveRDS(mamm_atys, "../Atys/mamm_atys.rds")
```
