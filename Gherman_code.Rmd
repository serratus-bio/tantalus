...
## First phase is done (1.14M files). Get Atys objects for them
...
viro_serratus <- getLoadedSummaryFiles(viro_sra@runInfo$Run, "../summary_files/") #8530 sras

viro_sra@runInfo <- viro_sra@runInfo %>% filter(Run %in% viro_serratus@header$sra)

viro_atys <- Atys(Serratus = viro_serratus,
                  SRA = viro_sra,
                  GENOME="")

saveRDS(viro_atys, "../Atys/viro_atys.rds")
...
mu_serratus <- getLoadedSummaryFiles(mu_sra@runInfo$Run, "../summary_files/") #8530 sras
