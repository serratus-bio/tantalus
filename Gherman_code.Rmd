...
summary_files <- paste(mu_sra@runInfo$Run, "summary", sep=".")
...
HITS_Family <- lapply(files_to_analyze, function(x){
  ...
  if (nrow(res) != 0){
    write.fst(res, paste("../temp_folder_mu_F/", sra, ".fst", sep=""), compress = 100)
  }
})
...
HITS_ACC <- lapply(files_to_analyze, function(x){
  ...
  if (nrow(res) != 0){
    write.fst(res, paste("../temp_folder_mu_Acc/", sra, ".fst", sep=""), compress = 100)
  }
})
...
DATA_PATH='../temp_folder_vert_F/'
sumWheres <- paste0( DATA_PATH,
                     system( paste0("ls ", DATA_PATH), intern = T ))

hits <- lapply(sumWheres, function(x){
  df_subset <- read.fst(x, c("sra","score", "pctid"))
  return(df_subset)
})
...
test <- hits#[1:10000]
#x <- do.call(rbind.data.frame, test) #12 secs 63256     3
suppressWarnings(y <- bind_rows(test)) #1.5 sec 63256     3
...
library(compare)
comparison <- compare(x,y,allowAll=TRUE)

difference <-
   data.frame(lapply(1:ncol(x),function(i)setdiff(x[,i],comparison$tM[,i])))
colnames(difference) <- colnames(x)
```
