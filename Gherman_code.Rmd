...
```{r}
#TEST
Borna <- readDfSQL(con, "FamilySections", family = "Coronaviridae",
                 columns = c("Sra","Score", "PctId", "Family"),
                 score = 64,
                 dataframe = T)
colnames(Borna) <- tolower(colnames(Borna))

family_df <- Borna %>% filter(family %in% c("Coronaviridae"))
family_df$score <- factor(family_df$score, levels = 0:100)
...
#cc <- scales::seq_gradient_pal("black", "yellow", "Lab")(seq(0,1,length.out=101))
cc <- viridis(101)
names(cc) <- 0:100
p <- ggplot(family_df, 
