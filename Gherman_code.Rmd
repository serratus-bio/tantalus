...
Viro_RNA$score_intervals <- test2

tbl <- table(Viro_RNA$family)
tbl <- sort(tbl, decreasing=T)

x <- Viro_RNA %>% group_by(pctid,score_intervals, family) %>% summarize(n=n(), log10n = log10(n() + 1))
...
```{r}
#try to fix the bar plot - doesn't work
x <- Viro_RNA %>% group_by(pctid,score_intervals, family) %>% summarize(n=n(), log10n_old = log10(n+1))
x <- x %>% filter(family == "Coronaviridae")
sums <- aggregate(n ~ pctid, x, sum)
y <- sums$n
names(y) <- sums$pctid
x$sums <- y[as.character(x$pctid)]
x <- transform(x, perct= n/sums)
x <- transform(x, log10n = log10n_old*perct)
