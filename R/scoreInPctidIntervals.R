...
#' @param bin_percent if split percent_identity into bins for plotting
#'
#' @return ggplot
#'
#' @examples
...
scoreInPctidIntervals <- function(family_df, family_name=NULL, title="", scale_log = F, bin_percent = T){
  ...
  if (bin_percent){
    Pctid_bins <- sapply(family_df$percent_identity, function(x){
      if ((x <= 100) & (x > 97)){
        return("97-100")
      } else if ((x <= 97) & (x > 94)){
        return("94-97")
    ...
    family_df$pctid_intervals <- factor(family_df$percent_identity)
  }
  
  # Reform DF around pctid_intervals
  family_df <- family_df %>% group_by(pctid_intervals, score, family_name) %>% summarize(n=n(), log10n = log10(n() + 1))
